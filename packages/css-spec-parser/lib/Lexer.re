...
let identifier_start_code_point = [%sedlex.regexp?
  'a' .. 'z' | 'A' .. 'Z' | non_ascii_code_point | non_ascii | '_'
];
...
let ident_char = [%sedlex.regexp?
  '_' | 'a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '-' | non_ascii
  | non_ascii_code_point | escape
];
  ...
  let consume_ident_like = buf => {
    ...
    | _ =>
     // TODO: from [Lexer.re]| | _ => Ok(IDENT(string))
      is_tag(string) ? Ok(TAG(string)) : Ok(IDENT(string))
    };
  };
};
...
// FIXME: unify [tokenizer] and [consume]
...
// FIXME: Unify, and move to the top of the file.
module Sedlexing = Lex_buffer;

// FIXME: Move exceptions to the top of the file.
 ...
 * locations. */
exception ParseError((Tokens.token, Lexing.position, Lexing.position));
...
// FIXME: Duplicate
  ...
  | ',' => COMMA
  | variable =>
    INTERPOLATION(latin1(~skip=2, ~drop=1, buf) |> String.split_on_char('.'))
  ...
  | ('#', name) => HASH_FROM_CSS_LEXER (latin1(~skip=1, buf))
...
and get_dimension = (n, buf) => {
  ...
  | 'n' => DIMENSION_FROM_CSS_LEXER (n, latin1(buf))
  | _ => NUMBER_FROM_CSS_LEXER (n)
  };
}
...
let token_to_string =
  ...
  | AT_RULE_STATEMENT(s)
  | AT_RULE(s) => "@" ++ s
  | HASH_FROM_CSS_LEXER(s) => "#" ++ s
  | NUMBER_FROM_CSS_LEXER (s) => s
  | UNICODE_RANGE(s) => s
  | FLOAT_DIMENSION(n, s) => n ++ s
  | DIMENSION_FROM_CSS_LEXER (n, d) => n ++ d
...
let parse = (skip_whitespaces, buf, parser) => {
  ...
  let last_token = ref((EOF, Lexing.dummy_pos, Lexing.dummy_pos));
